...
local function add_player(cv, env)
    ...
    function player:add_params()
        ...
        params:add{type = "option", id = "crow_eval_output"..self.ext, name = "eval output", options={1,3}, default=1, action=function(value)
          nb_crow_ext = self.ext
          osc.send( { "localhost", 57120 }, "/sc_crooner/set_crow_output",{value})
        end}

        params:add{type = "number", id = "target_conf"..self.ext, name = "target conf level", min=1, max=100, default=50,formatter=function(param) return param:get() .. "%" end}
        params:add{type = "number", id = "current_conf"..self.ext, name = "current conf level", min=1, max=100, default=50,formatter=function(param) return param:get() .. "%" end}
        params:add_control("current_freq"..self.ext, "current freq", controlspec.new(20, 4000, 'exp', 0, 440, 'Hz', 0.0003))
        params:set_action("current_freq"..self.ext, function() end)
...
mod.hook.register("script_post_init", "nb crow post init", function()
  ...
  function osc.event(path,args,from)
    ...
    elseif path == "/lua_crooner/pitch_confidence" then
      ...
      current_freq=args[1]
      current_conf=args[2]
      if current_freq  and tonumber(current_conf) >= params:get("target_conf"..nb_crow_ext)*0.01 then
        params:set("current_freq"..nb_crow_ext,current_freq and current_freq)
      elseif current_freq then
        params:set("current_freq"..nb_crow_ext,-1)
      end
